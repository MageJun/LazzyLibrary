...
public class PermissionHelper {
    ...
    private static int BASE_CODE = 200;

    public static int obtionCode(){
        return BASE_CODE++;
    }
    ...
    public final static int CAMERA_CODE = 103;

    public final static int ACCESS_FINE_LOCATION_CODE = 104;

    public final static int CALL_CODE = 105;

    public final static int CONTACTS_CODE = 106;
...
//            new PermissionModel("电话", Manifest.permission.READ_PHONE_STATE, "我们需要读取手机信息的权限来标识您的身份", READ_PHONE_STATE_CODE),
            ...
            new PermissionModel("地图定位", Manifest.permission.ACCESS_COARSE_LOCATION,"我们需要权限来获取您当前的位置",ACCESS_FINE_LOCATION_CODE),
            new PermissionModel("联系人", Manifest.permission.READ_CONTACTS,"我们需要权限读取联系人，方便您快速填写联系人信息",CONTACTS_CODE),
            new PermissionModel("相机", Manifest.permission.CAMERA,"我们需要权限来使用系统相机",CAMERA_CODE)
//            ,new PermissionModel("摄像头", Manifest.permission.CAMERA, "我们需要您允许我们读写你的存储卡，以方便我们临时保存一些数据",
////                    WRITE_EXTERNAL_STORAGE_CODE)
    };
    ...
    /**
     * 在Android 6.0+上运行时申请单个权限
     */
    public boolean requestPermission(PermissionModel model) {
        try {
            ...
            Log.e("PermissionHelper", e.getMessage());
        }
        return true;
    }
