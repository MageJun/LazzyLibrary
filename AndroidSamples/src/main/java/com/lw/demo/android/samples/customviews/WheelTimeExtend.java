...
public class WheelTimeExtend {
    ...
    public WheelTimeExtend(View view, boolean[] type, int gravity, int textSize) {
        super();
        this.view = view;
        this.type = type;
    ...
    private void setSolar(int year, final int month, int day, int h, int m, int s) {
      ...
      /*  if (startYear == endYear && startMonth == endMonth) {
            if (list_big.contains(String.valueOf(month + 1))) {
                ...
                wv_day.setAdapter(new NumericWheelAdapterExtend(year,month,startDay, endDay));
            ...
            if (list_big.contains(String.valueOf(month + 1))) {
                ...
                wv_day.setAdapter(new NumericWheelAdapterExtend(year,month,startDay, 31));
            } else if (list_little.contains(String.valueOf(month + 1))) {

                wv_day.setAdapter(new NumericWheelAdapterExtend(year,month,startDay, 30));
            } else {
                // 闰年
                if ((year % 4 == 0 && year % 100 != 0) || year % 400 == 0) {

                    wv_day.setAdapter(new NumericWheelAdapterExtend(year,month,startDay, 29));
                } else {

                    wv_day.setAdapter(new NumericWheelAdapterExtend(year,month,startDay, 28));
                }
            }
            wv_day.setCurrentItem(day - startDay);
            ...
            if (list_big.contains(String.valueOf(month + 1))) {
                ...
                wv_day.setAdapter(new NumericWheelAdapterExtend(year,month,1, endDay));
            } else if (list_little.contains(String.valueOf(month + 1))) {
                if (endDay > 30) {
                    endDay = 30;
                }
            ...
            if (list_big.contains(String.valueOf(month + 1))) {
                ...
                wv_day.setAdapter(new NumericWheelAdapterExtend(year,month,1, 31));
            } else if (list_little.contains(String.valueOf(month + 1))) {

                wv_day.setAdapter(new NumericWheelAdapterExtend(year,month,1, 30));
            } else {
                // 闰年
                if ((year % 4 == 0 && year % 100 != 0) || year % 400 == 0) {

                    wv_day.setAdapter(new NumericWheelAdapterExtend(year,month,1, 29));
                } else {

                    wv_day.setAdapter(new NumericWheelAdapterExtend(year,month,1, 28));
                }
            }
            wv_day.setCurrentItem(day - 1);
        }*/
        wv_day.setAdapter(new NumericWheelAdapterExtend(this.startDate.getTime(),this.endDate.getTime()));
        wv_day.setCurrentItem(differentDays(year,month,day));
        wv_day.setGravity(gravity);
        //时
        wv_hours = (WheelView) view.findViewById(R.id.hour);
        wv_hours.setAdapter(new NumericWheelAdapter(0, 23));
...
//                maxItem = endD;
    ...
    public Date getTimeOnlyDay(){
        int index = wv_day.getCurrentItem();
        NumericWheelAdapterExtend.Data data = (NumericWheelAdapterExtend.Data) wv_day.getAdapter().getItem(index);
        if(data!=null){
            Calendar calendar = Calendar.getInstance();
            calendar.setTime(data.getTime());
            int hour = wv_hours.getCurrentItem();
            int minutes = wv_minutes.getCurrentItem();
     ...
     * @return
     */
    private long getMonthTimeDate(long timeSample){
        Calendar calendar = Calendar.getInstance();
    ...
    private int differentDays(int year,int month,int day){
        Calendar calendar = Calendar.getInstance();
        ...
        {
            int timeDistance = 0 ;
            int min = Math.min(year1,year);
            int max =  Math.max(year1,year);
